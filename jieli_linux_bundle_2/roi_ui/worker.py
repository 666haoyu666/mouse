...
class UdpInferWorker(QThread):
        ...
        for path in candidates:
            if path.exists():
                ...
                if spec and spec.loader:
                    ...
                    # 关键修复：dataclass 需要能从 sys.modules 找到当前模块
    ...
    def _decode_and_infer(self, payload: bytes):
        if not payload.startswith(b'\xFF\xD8') or b'\xFF\xD9' not in payload:
            ...
            if self._bad_jpeg_count <= 10:
                self._log(
                    f'[DBG] 非JPEG帧 #{self._bad_jpeg_count}, '
                    f'len={len(payload)}, head={payload[:8].hex()}, tail={payload[-8:].hex()}'
                )
            return
    ...
    def _parse_udp_packet(self, packet: bytes, addr: Tuple[str, int]):
        ...
        if self.config.device_ip and addr[0] != self.config.device_ip:
            self._log(f'[DBG] 丢弃非目标IP UDP: from={addr}, expected={self.config.device_ip}')
            return
        pos = 0
        plen = len(packet)
